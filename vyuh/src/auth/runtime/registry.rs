...
pub(super) fn validate_new(
    ...
    let id = ProviderId::new(value.name.as_str())?;
    if existing.iter().any(|provider| provider.id() == &id) {
        return Err(AuthError::DuplicateProvider(id.to_string()));
    }
    validate_access_selector(value, existing)?;
    validate_refresh_selector(value, existing)
}

fn validate_access_selector(
    value: &ProviderDefinitionInner,
    existing: &[ProviderRuntime],
) -> Result<(), AuthError> {
    let selector = value.kind.access_location().selector();
    if existing.iter().any(|provider| {
        provider.access_location().selector() == selector
            || provider
                .refresh_location()
                .is_some_and(|other| other.selector() == selector)
    }) {
        return Err(AuthError::AmbiguousProvider(selector));
    }
    Ok(())
}

fn validate_refresh_selector(
    value: &ProviderDefinitionInner,
    existing: &[ProviderRuntime],
) -> Result<(), AuthError> {
    let Some(location) = value.kind.refresh_location() else {
        return Ok(());
    };
    let selector = location.selector();
    if existing.iter().any(|provider| {
        provider.access_location().selector() == selector
            || provider
                .refresh_location()
